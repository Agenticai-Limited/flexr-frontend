...
export const QAChatInterface = ({ userInfo }: QAChatInterfaceProps) => {
  ...
  useEffect(() => {
    const shuffledQuestions = [...commonQuestions].sort(
      () => 0.5 - Math.random()
    );
  ...
  const handleSendMessage = async (
    ...
    try {
      ...
      setMessages((prev) =>
        prev.map((msg) =>
          msg.id === assistantMessageId ? { ...msg, id: message_id } : msg
        )
      );

      const eventSource = new EventSource(
        `${BASE_URL}/api/task-progress/${message_id}`
      );
      ...
      setMessages((prev) =>
        prev.map((msg) =>
          msg.id === assistantMessageId
            ? {
                ...msg,
                content: "Failed to start the task. Please try again.",
                metadata: { isStreaming: false },
              }
            : msg
        )
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white shadow-soft overflow-hidden">
        ...
        {showCommonQuestions && (
          <div className="px-4 pt-2 pb-1 border-t border-gray-100">
            <h3 className="text-left text-sm font-semibold text-gray-600 mb-3">
          ...
          <div className="flex items-end space-x-4">
            ...
            >
              ...
              <button
                type="submit"
                disabled={
                  isLoading || !(inputMessage.trim() || uploadedFile?.url)
                }
